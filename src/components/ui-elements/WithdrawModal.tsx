  ...
  const { data: forexRates, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // For testing, let's use some sample FOREX rates
        const sampleRates = {
          USD: 1,
          EUR: 0.92,
          GBP: 0.79,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="glass neon-border">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">Withdraw USDC</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            {/* USDC Amount Input */}
            <div>
              <label className="text-sm font-medium text-gray-400">How much USDC do you want to withdraw?</label>
              <Input
                type="number"
                value={usdcAmount}
            ...
            {/* Currency Selection */}
            <div>
              <label className="text-sm font-medium text-gray-400">Select your withdrawal currency</label>
              <Select
            ...
            <div className="glass p-4 rounded-lg space-y-2">
              ...
              <div className="text-sm text-gray-400">
                Exchange rate: 1 USDC = {forexRates?.[selectedCurrency]?.toFixed(4) || '...'} {selectedCurrency}
              </div>
